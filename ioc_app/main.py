...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def on_ai_done(self, analysis):
        self.progress_bar.setVisible(False)
        # parse markdown bullets into definition list
        entries = re.findall(r"\d+\.\s*\*\*(.+?)\*\*[:]*\s*(.+)", analysis)
        dl_html = "".join(
            f"<dt style='font-weight:bold;margin-top:8px;'>{key}</dt>"
            f"<dd style='margin:0 0 8px 20px;color:#E0F2E9;'>{value}</dd>"
            for key, value in entries
        )
        self.output_area.insertHtml(f"""
            <hr style="border:1px solid #226622;">
            <h2 style="color:#33CC33;">🤖 AI Threat Summary</h2>
            <dl style="margin-left:20px;color:#E0F2E9;">
              {dl_html}
            </dl>
        """)
        self.finish_scan()
